...
**Problem Statement:** This analysis addresses the challenge of predicting whether an individual's annual income exceeds $50,000 based on readily observable demographic and employment characteristics. Understanding which features most strongly predict income level is crucial for multiple stakeholders: policymakers designing targeted economic assistance programs, educators planning workforce development initiatives, and researchers studying economic mobility and inequality.

**Importance and Applications:** Accurate income prediction models serve several important purposes. First, they help identify populations at risk of economic hardship, enabling more efficient allocation of social services and support programs. Second, they provide insights into the structural factors that drive income differences, informing evidence-based policy decisions around education access, job training, and labor market interventions. Third, such models can reveal which combinations of characteristics are most predictive of crossing critical income thresholds, helping individuals and career counselors make informed decisions about education and career paths. Finally, by quantifying the relative importance of various demographic and employment factors, these models contribute to our understanding of economic inequality and social mobility in the United States.

**Research Question:** Specifically, we seek to answer: Can demographic characteristics (age, sex, marital status, education) and employment-related variables (occupation, work hours, employment type) reliably predict whether someone earns above or below $50,000 annually? Furthermore, which of these features carry the most predictive power, and can simple linear models capture these relationships, or are more complex non-linear approaches necessary?
...
The following classification algorithms are considered for predicting whether an individual's annual income exceeds $50,000: Decision Tree, k-Nearest Neighbors (k-NN), Support Vector Machine with a radial basis function kernel (SVM-RBF), Logistic Regression, and Gaussian Naive Bayes. With the exception of the features described above (`fnlwgt`, `education-num`, and `race`), all available features are used to fit the models.

The dataset is split such that 80% of the observations are allocated to the training set and 20% are reserved for the test set. This split balances the need for sufficient training data with the ability to evaluate model performance on unseen observations. Missing values are handled through imputation, while categorical variables are transformed using a combination of one-hot encoding and ordinal encoding, depending on their characteristics. For model selection, we first perform 5-fold cross-validation across all candidate models using default hyperparameter settings. Based on cross-validation performance, Logistic Regression and SVM-RBF are selected as the top-performing models. These models are then further optimized using random hyperparameter search with 5-fold cross-validation, using classification accuracy as the primary evaluation metric. Finally, the best estimators obtained from this tuning process are evaluated on the unseen test dataset to assess their generalization performance.
